...
class about_section extends Controller
{
    ...
    {
        // 1. Validate dữ liệu với thông báo tiếng Việt
        $request->validate([
    ...
    public function update_section(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'image_main' => 'nullable|image|mimes:jpg,jpeg,png,gif|max:10048',
            'shapes' => 'nullable|array',
            'shapes.*' => 'image|mimes:jpg,jpeg,png,gif|max:10048',
        ], [
            'title.required' => 'Tiêu đề không được để trống',
            'title.max' => 'Tiêu đề tối đa 255 ký tự',
            'description.required' => 'Mô tả không được để trống',
            ...
            'shapes.*.image' => 'Các file shapes phải là ảnh hợp lệ',
            'shapes.*.mimes' => 'Các file shapes chỉ nhận định dạng jpg, jpeg, png, gif',
            'shapes.*.max' => 'Các file shapes không được lớn hơn 10MB',
        ]);

        // Lấy bản ghi hiện tại
        $section = DB::table('about_sections')->where('id', $id)->first();

        // Xử lý upload ảnh chính nếu thay đổi
    ...
    public function delete_section($id)
    {
        ...
        // Xóa dữ liệu + (tuỳ chọn xóa file ảnh)
