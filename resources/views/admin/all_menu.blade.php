                            ...
                            @foreach($menus as $menu)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('menus.destroy', $menu->id) }}" method="POST" style="margin-top:15px;" onsubmit="return confirm('Bạn có chắc muốn xóa menu này?')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                @foreach($childMenus as $child)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('menus.edit', $child->id) }}" class="btn btn-primary btn-sm">
                                                <i class="fa fa-edit"></i> Sửa
                                            </a>
                                            <form action="{{ route('menus.destroy', $child->id) }}" method="POST" style="margin-top:15px;" onsubmit="return confirm('Bạn có chắc muốn xóa menu này?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm">
                                                    <i class="fa fa-trash"></i> Xóa
